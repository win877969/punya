...
<a href='v2rayng://install-config?url=${encodeURIComponent(subbestip)}' target='_blank'>v2rayNG Best ip</a></p>`;
  ...
  <head>
	...
	<meta name='twitter:title' content='Bedebah - วเลส configuration and subscribe output' />
...
function createValueSub(userID_path, hostname) {
	...
	const result = arrayUserID.flatMap((UserID) => {
		const Httpconfiguration = Array.from(HttpPortset).flatMap((Port) => {
			if (!hostname.includes('pages.dev')) {
				const partUrl = `${hostname}-HTTP-${port}`;
				const mainvalueHttp = atob(pt) + '://' + userid + atob(at) + hostname + ':' + port + commonUrl partHttp + Url part;
...
const cn_hostnames = [
	...
	// "zula.com",
	// "telewebion.com",
	'https://punya.bmkg.xyz/8f4e4a98-0d82-4f4a-a74d-9b547e737594',
	// 'alibaba.ir',
	// 'soft98.ir',
	// 'yasdl.com',
	// 'upload.ir',
